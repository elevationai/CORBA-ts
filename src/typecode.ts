...
export class TypeCode {
  ...
  member_count(): number {
    ...
    const members = this._params.get("members") as Array<any>;
   ...
   */
  set_param(name: string, value: any): void {
  ...
  private check_method_validity(method: string): void {
    ...
    if (!valid_methods.includes(method)) {
      throw new CORBA.BAD_PARAM(`Operation ${method} not valid for TypeCode of kind ${TypeCode.Kind[this._kind]}`);
    }
  }
...
export namespace TypeCode {
   ...
   */
  export function create_struct_tc(id: string, name: string, members: Array<{name: string, type: TypeCode}>): TypeCode {
